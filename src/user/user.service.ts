...
export class UserService {
  ...
  async register(user: RegisterUserDto) {
    const captcha = await this.redisService.get(`captcha_${user.captcha}`);
    if (!captcha) {
  ...
  async login(loginInfo: LoginUserDto, isAdmin: boolean) {
    ...
    if (md5(loginInfo.password) !== foundUser.password) {
  ...
  async findUsersByPage(pageNo: number, pageSize: number, params) {
    const skipCount = (pageNo - 1) * pageSize;
    const { email, username, nickName } = params;
